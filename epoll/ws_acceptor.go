...
func (my *WsAcceptor) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	var item = newWsConn(conn, my.watcher, my.receivedChanSize)
	if item != nil {
...
func (my *WsAcceptor) goWatcher(watcher *gaio.Watcher) {
	...
	for !my.IsClosed() {
		...
		for _, item := range results {
			...
			case gaio.OpRead:
				if playerConn, ok := item.Context.(*WsConn); ok {
					if item.Size > 0 {
...
func (my *WsAcceptor) Close() error {
	atomic.StoreInt32(&my.isClosed, 1)
	return nil
}

func (my *WsAcceptor) IsClosed() bool {
